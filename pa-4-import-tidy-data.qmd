...
library(readr)  #load the package from the video that cleans variable names
library(janitor)
```
...
youth <- read_csv("data-raw/YRBS2015.csv", skip=1, na =c(".","N/A"), col_types = list(AgeCat = "n", Grade = "n", `Perception of weight` = col_factor(levels = c("Very underweight","Slightly underweight", "About the right weight", "Slightly overweight", "Very overweight")), Gender = "c", `Height in meters` = "d", `Weight in kilograms` = "d", `Body Mass Index` = "d", `BMI percentile` = "n", `Fruit eating` = "c", `Salad eating` = "c", `Other vegetable eating` = "c", `Soda drinking` = "c", `Breakfast eating` = "n", `Physical activity >= 5 days` = "n", `Television watching` = "c", `Sports team participation` = "n", .default = "?"))
```

Describe some of the issues you notice in the data:

> For age cat there is a lot of ".", NA is being read as a character, column names are in the wrong spot.
...
write_csv(x=youth_clean, file = "data-clean/youth_clean.csv")
```

Why do we keep the code for the data cleaning but set the code chunks to `eval: false`?

> So that the chunks aren't included in our rendered quarto qmd file. in R studio on Rachels computers
...
> Difficulty concentrating and how well speak english are two of the most missing variables, and the most missing combination is of drinks, oral, sexual, english, concentration.

How does the missingness of the data impact what variables you might choose to analyze? Why?

> The ones with more missingness are less accurate and I refuse to analyze missing data(simply not possible).
...
youth_clean |>  #instead of data = youth_clean in ggplot we can "pipe" our data  
  ...
  ggplot(aes(x= `height_in_meters`, y = weight_in_kilograms, color= perception_of_weight)) +
  labs(title= "Weight and Height Distribution Among Different Grades and Genders", x= " Height (m) ", y=" Weight (kg)") +
...
youth_clean |>  #instead of data = youth_clean in ggplot we can "pipe" our data  
  drop_na() |>  #drops missing values
  ggplot(aes(x= `height_in_meters`, y = weight_in_kilograms, color= perception_of_weight)) +
  labs(title= "Weight and Height Distribution Among Different Grades and Genders", x= " Height (m) ", y=" Weight (kg)", color = "Perception of Weight") +
  geom_point(alpha=0.25) +
  geom_smooth(method = "lm")+
  scale_color_viridis_d(option = "mako", begin = 0.2, end = 0.85) +
  theme_minimal() + 
  theme(legend.position = "bottom") + 
  facet_wrap(grade~gender, ncol = 4) 
